...
######## simulate dat1 using D (May get error due to R version, see alterative solution below.)########
dat1 <- sim(D, n = 1000)
dat1$ExamDifficulty <- as.numeric(dat1$ExamDifficulty)

#### combine simulated dat1 with vector house_price ####
dat1 <- cbind(dat1, house_prices)
summary(dat1)

######## create dat with classified ExamPerformance ####
dat <- dat1
...
######## !!!!! alternative solution for getting simulated data due to the version problem of R ##########
dat1 <- read_csv("ExamPerformance2.csv")
dat <- read_csv("ExamPerformance.csv")
dat$EPClass <- as.factor(dat$EPClass)
##### see data structure ####
summary(dat)
#### correlation plot ####
